...
const EditJob = () => {
  ...
  const deleteJobData = async () => {
    ...
    if (resp.data.status === 1) {
      setstatusMessage(resp.data.msg + '<a href="/">Back to home page.</a>')
    }
  }
  const deleteJob = () => {
            ...
            <div className="mt-5 flex flex-wrap justify-center">
              <div className={`overflow-hidden shadow rounded-md" ${formOverLay === 1 ? "form-overlay" : ""}`}>
                ...
                <div className="flex justify-between bg-gray-50 px-4 py-3 sm:px-6">
                  <Link to="#" onClick={deleteJob} className="mt-3 bg-brightRed text-white font-bold text-underline-none p-2 block hover:bg-Green-100 md:px-4 py-2 inline-block ">
                    Delete
                  </Link>
                  &nbsp;
