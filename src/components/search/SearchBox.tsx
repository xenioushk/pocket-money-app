...
const SearchBox = () => {
  ...
  const showResults = (searchResults?.pagination?.total ?? 0) > 0
  const showNoResults = !isLoading && searchText.length >= 3 && (searchResults?.pagination?.total ?? 0) === 0
    ...
    <div className="container px-4 mx-auto items-center mt-4 md:px-0">
      <form className="flex items-center mx-auto mt-8 mb-8 md:w-2/3" onSubmit={onSubmit}>
        ...
        <div className="relative w-full flex">
          ...
          <div className={`flex absolute inset-y-0 right-2 items-center pl-3 pointer-events-none  ${showSpinner ? "" : "hidden"}`}>
          ...
          <input value={searchText} onChange={(e) => setSearchText(e.target.value)} type="text" id="jobSearch" className="bg-gray-50 border-2 border-gray-300 text-gray-900 text-sm rounded focus:border-gray-500 focus:border-gray-500 block w-full pl-10 p-2.5" placeholder="Search Job" />

          <div className="absolute w-full mt-12">
            {showNoResults && <div className="justify-center text-center w-full">No results found</div>}
            <div className=" block">
              {showResults && searchResults ? (
                <div className="justify-center w-full">
                  <ul className="bg-white bg-gray-50 border rounded border-gray-300">
                    {searchResults.data.map((job, index) => (
